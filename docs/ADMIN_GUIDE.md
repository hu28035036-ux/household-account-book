...
3. **첫 관리자 이메일을 미리 `allowed_emails`에 INSERT** (Dashboard SQL Editor):
   ...
   ```
4. 첫 매직링크 로그인 → `/admin` 메뉴(사이드바 가장 아래)가 보이면 OK.

## 일상 운영
### 새 사용자 초대
1. `/admin`의 **이메일 화이트리스트**에 추가
2. 사용자에게 가입 페이지(`/login`) 링크 전달 + "이 이메일로 매직링크 받으세요" 안내
3. 매직링크 클릭 → 가입 트리거가 화이트리스트 검증 → 통과 시 기본 카테고리/결제수단 자동 시드
4. 미등록 이메일은 **EMAIL_NOT_ALLOWED** 에러로 차단됨 (UI에 친절한 메시지)
